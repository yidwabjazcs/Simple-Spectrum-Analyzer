...
uint8_t pRxBuffer;
uint8_t RxFlag = 0;
uint8_t three_ff [3] = {0xff,0xff,0xff};

int8_t first_ADC_flag = -1;//��һ�ν���ADC������־
...
uint8_t count_spurious;//��ɢ������
uint8_t spuriousThreshold;//*0.02

uint16_t peak_freq;//�����ȶ�ӦƵ��
uint8_t peak;//������
uint8_t  index_peak_freq;//�����ȶ�Ӧ�±�

#define WAIT_LOCK_COUNT  6

uint32_t temp =0 ;
uint32_t temp_NowFre = 0;

uint16_t v_PLL = 600;//���໷Ĭ�����
double log_v_PLL ;//log10���໷Ĭ�����

uint8_t v_input = 10;//��ʼ��10mV
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    if(first_ADC_flag != -1)
    {
      ...
      else
      {
        ...
        ADC_Buffer [pADC_Buffer] = bar_value;
        ...
        if(pADC_Buffer == 201)
        {
          ...
          printf("n0.val=%d\xFF\xFF\xFF", count_spurious);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	{
		lock_time_flag=0;
		printf("x1.val=%d\xFF\xFF\xFF",lock_time);
      ...
      switch (state)
      {
        ...
        case 4:
        {
          ...
          if(pRxBuffer == 2)
          {
            ...
            NowFre=temp * 100;
            ADF4351WriteFreq((float)temp / 10.0);
          }
          break;
        }
        case 5:
